...
def ca_from_angles(angles, bond_len=3.80): 
    ...
    #print(rot_mats.shape, frames[0].shape)
    #print(rot_mats.type())
    #print(frames[0].type())
    rot_mats = rot_mats.double()
    #print(rot_mats.type())
...
####### METRICS ############
############################
def compute_dist(atom_a, atom_b):
    return torch.norm(atom_a - atom_b)

def dRMSD(true_coords, pred_coords):
    # Input Shape: true_coords (b, l, c, 3)
    #              pred_coords (b, l, c, 3)
    true_alpha_trace = true_coords[:, :, 1, :]
    pred_alpha_trace = pred_coords[:, :, 1, :] # (b, l, 3)
    #true_coords = rearrange(true_coords, "... c d e -> ... (c d) e")
    #pred_coords = rearrange(pred_coords, "... c d e -> ... (c d) e")
    
    length = true_alpha_trace.shape[1]
    drmsd = 1e-10
    for b in range(true_alpha_trace.shape[0]):
        for i in range(length-1):
            for j in range(i, length):
                
                true_ = compute_dist(true_alpha_trace[b, i, :], true_alpha_trace[b, j, :])
                pred_ = compute_dist(pred_alpha_trace[b, i, :], pred_alpha_trace[b, j, :])
                drmsd += torch.pow(true_ - pred_, 2)
                #drmsd += (compute_dist() - compute_dist()  ** 2
    return torch.sqrt(drmsd / (length * (length - 1))) 
def RMSD(true_coords, pred_coords):
    return torch.sqrt(torch.sum((true_coords - pred_coords) ** 2) / (true_coords.shape[1] * true_coords.shape[1]))
def get_protein_metrics(
    ...
    #RMSD = torch.sqrt(torch.sum((true_coords - pred_coords) ** 2) / (true_coords.shape[1] * true_coords.shape[1])) # RMSD calc 
    #dRMSD = dRMSD_calc(true_coords, pred_coords)
    ...
    # pred_coords -> add cloud mask -> transpose -> rotation align -> transpose back

    #true_aligned, pred_aligned = Kabsch(
    #    pred_coords[cloud_mask].t(), true_coords[cloud_mask].t()
    #)
    true_aligned, pred_aligned = true_coords, pred_coords
    ...
    #true_coords[cloud_mask] = true_aligned
    #pred_coords[cloud_mask] = pred_aligned
